...
class _NotificationsScreenState
  ...
  bool _isToday(DateTime date) {
    return _getDateOnly(date) ==
        _getDateOnly(DateTime.now());
  }
  ...
  bool _isThisWeek(DateTime date) {
    ...
    return !itemDate.isBefore(weekStart) &&
        !itemDate.isAfter(weekEnd);
  }
  ...
  List<NotificationItem> _getFilteredNotifications() {
    List<NotificationItem> filtered =
        List.from(AppData.notifications);
  ...
  Map<String, List<NotificationItem>>
    ...
    for (NotificationItem item
        in sortedNotifications) {
      String label =
          _getDateLabel(item.createdAt);
  ...
  void _deleteNotification(
    ...
    // Remove from shared AppData
    setState(() {
      AppData.notifications.removeWhere(
        (notification) =>
    ...
    // Darker blue for UNDO in dark mode
    final Color undoColor = isDarkMode
        ? AppColors.blue
        : Theme.of(context)
            .colorScheme
            .primary;
    ...
    // Make sure the SnackBar disappears
    // after 4 seconds.
    Future.delayed(
  ...
  Widget _buildEmptyState(
    ...
    if (_selectedStatus == 'Unread') {
      ...
      message =
          'No notifications for this time';
    }
